...
n = 20      # number of particles should be even
...
def progress_list():
    ...
    noise = np.sqrt(6.0*gamma*Tb/dt)
    fac = np.sqrt(ke/kinetic_sum)
    calculate_LJ_between_all()
    forces = forces * noise * random.random()
    vel = ((vel + forces / mass * dt) * gfric) * fac
    pos += vel * dt
...
while step < steps:
    ...
    if ((step % saveInterval) == 0):
        ...
        for i in range (len(pos)) :
            origo = [0,0,0]
            distance = abs(pos[i] - origo)
            scalarDistance = (distance[0]**2+distance[1]**2+distance[2]**2)**(1/2)
    ...
    #for i in range(n):    gfric = 1.0-gamma*dt/2.0
    #noise = np.sqrt(6.0*gamma*Tb/dt)
    #fac = np.sqrt(ke/kinetic_sum)
    ...
    kinetic_particle = np.sum(E_kinetic, axis=1)
    kinetic_sum = np.sum(kinetic_particle)
    # ke_adj = resamplekin(kinetic_sum,ke,6,tau)
